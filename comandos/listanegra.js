    ...
    try {
        ...
        if (fs.existsSync(filePath)) {
            ...
            if (listanegraContent.trim() !== '') {
                // Remove os sufixos @c.us ou @g.us dos números
                const formattedContent = listanegraContent
                    .split('\n')
                    .map(number => number.replace('@c.us', '').replace('@g.us', ''))
                    .join('\n');
...
if (messageContent.startsWith('!addln ')) {
    try {
        ...
        // Mantém o sufixo '@c.us' ou '@g.us' no número
        const sanitizedNumber = cleanNumber.includes('@c.us') || cleanNumber.includes('@g.us') 
            ? cleanNumber 
            : cleanNumber + '@c.us';
...
if (messageContent.startsWith('!rln ')) {
    try {
        ...
        // Adiciona o sufixo '@c.us' ao número
        const sanitizedNumber = numberToRemove + '@c.us';
...
if (messageContent.startsWith('!banln')) {
    try {
        ...
        const mentionedId = mentions[0].replace('@s.whatsapp.net', '@c.us');
        ...
        try {
            ...
            if (!existingNumbers.includes(mentionedId)) {
                // Adiciona o ID ao arquivo com o formato correto (@c.us)
